"""
src/gui/pages/risk.py — Risk profile page.
...
Internal engine guardrails are shown as a read-only status card at the
bottom.  Users cannot edit them here.
"""
...
from src.gui.components import SectionCard, ActionBanner
...
class RiskPage(ctk.CTkScrollableFrame):
    ...
    def _build(self) -> None:
        page_header(self, "Risk Profile", "Control how much your account can lose before trading stops")
        ...
        _risk_field(
            ...
            unit="%",
            tip="AQ Agent stops trading for the day once this percentage of your account balance is lost.\n"
                "Recommended: 0.5% – 10%.",
        ...
        _risk_field(
            ...
            unit="trades",
            tip="Worst-case consecutive losses used to divide the daily budget into per-trade risk.\n"
                "A higher streak means smaller risk per trade.  Recommended: 2 – 5.",
            vars_dict=self._vars,
            on_change=self._update_formula,
        )
        ...
        _risk_field(
            limits_card.body,
            label="Max Lot Size",
            key="risk.max_lot_size",
            unit="lots",
            tip="Hard cap on position size.  AQ Agent will never open a trade larger than this, "
                "regardless of account size or risk calculation.",
            vars_dict=self._vars,
        )
        ...
        left = ctk.CTkFrame(hedge_row, fg_color="transparent")
        left.pack(side="left", fill="x", expand=True)
        ctk.CTkLabel(
            left, text="Prevent Hedging", anchor="w",
            font=ctk.CTkFont(size=13), text_color=TEXT,
        ).pack(anchor="w")
        ctk.CTkLabel(
            left,
            text="Block opposite-direction trades on the same symbol.  "
                 "Recommended: On.",
        ...
        ctk.CTkSwitch(
            hedge_row, text="", variable=self._var_no_hedging,
            onvalue=True, offvalue=False,
            command=self._on_hedging_changed,
        ).pack(side="right")

        # Hedge warning banner — hidden by default
        ...
        section_rule(content, "ENGINE GUARDRAILS")
    ...
    def _on_hedging_changed(self) -> None:
        if not self._var_no_hedging.get():
            self._hedge_banner.show(
                ...
                "This can increase risk and make trade tracking harder.",
                "warn",
            )
        else:
            self._hedge_banner.hide()
    ...
    def _save(self) -> None:
        ...
        self._save_banner.show("Risk settings saved.", "good", auto_dismiss_after_ms=3000)
...
def _guard_row(parent: tk.Widget, label: str, detail: str) -> None:
    from src.gui.theme import GREEN, MUTED, TEXT, SURFACE_RAISED, LINE
    row = ctk.CTkFrame(parent, fg_color="transparent")
