; TradeRelay.iss — Inno Setup script (GUI wizard edition)
;
...
[Icons]
; ============================================================================
Name: "{group}\Reconfigure TradeRelay"; \
...
[Run]
; ============================================================================
; Open the TradeRelay dashboard after successful install (Finish page checkbox)
Filename: "https://app.traderelay.io"; \
    Description: "Open TradeRelay dashboard in browser"; \
...
begin
  ...
  Lines.Add('# TradeRelay Engine — configuration');
  Lines.Add('# Generated by installer on ' +
    GetDateTimeString('yyyy/mm/dd hh:nn:ss', '/', ':'));
  Lines.Add('# Re-run the "Reconfigure TradeRelay" shortcut to change any setting.');
  Lines.Add('');
  ...
  Lines.Add('  # password -> set MT5_PASSWORD in .env');
...
begin
  Lines := TStringList.Create;
  Lines.Add('# Secrets — do not commit to version control.');
  Lines.Add('# Generated by TradeRelay Engine installer.');
  Lines.Add('# Restricted to current Windows user account.');
  Lines.Add('');
  Lines.Add('TRADERELAY_ACTIVATION_KEY=' + TrimStr(ActivationKeyPage.Values[0]));
  Lines.Add('MT5_PASSWORD=' + MT5PassPage.Values[0]);
  Lines.Add('');
  Lines.Add('# HMAC-SHA256 for signal signature verification (optional)');
...
// ---------------------------------------------------------------------------
// Restrict .env file to the current user (icacls, equivalent to chmod 600)
// ---------------------------------------------------------------------------
procedure ProtectEnvFile(const FilePath: String);
var
  ResultCode: Integer;
begin
...
begin
  ...
  MT5Page := CreateInputQueryPage(
    ...
    'TradeRelay found the MT5 path below automatically. Correct it if needed, then fill in your account details.',
    ''
  );
  ...
  MT5PassPage := CreateInputQueryPage(
    ...
    'Your password is stored in .env on this machine only, protected by Windows file permissions. It is never sent to TradeRelay servers.',
    ''
  );
  ...
  ConnPage := CreateInputQueryPage(
    MT5PassPage.ID,
    'Gateway Connection',
    'Configure how this engine connects to the TradeRelay cloud gateway.',
    ''
  );
  ConnPage.Add('Gateway WebSocket URL:', False);
  ConnPage.Add('Engine ID (unique name for this machine):', False);
  ConnPage.Values[0] := 'wss://gateway.traderelay.io/engine';
...
begin
  ...
  if not SaveStringToFile(ConfigFile, BuildConfigYaml, False) then
    MsgBox(
      ...
      'Use the "Reconfigure TradeRelay" shortcut after install.',
      mbError, MB_OK
    );

  // Write .env and restrict permissions to current user
  if not SaveStringToFile(EnvFile, BuildEnvContent, False) then
    MsgBox(
      'Warning: could not write .env.' + #13#10 +
      'Use the "Reconfigure TradeRelay" shortcut after install.',
      mbError, MB_OK
    )
  else
    ProtectEnvFile(EnvFile);
...
begin
  if CurPageID = wpFinished then
    WizardForm.FinishedLabel.Caption :=
      'TradeRelay Engine has been installed successfully.' + #13#10 + #13#10 +
      'Click Finish to open the TradeRelay dashboard in your browser.' + #13#10 + #13#10 +
      'Tip: use the "Reconfigure TradeRelay" Start Menu shortcut' + #13#10 +
